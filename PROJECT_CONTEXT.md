...
以后修改本项目时，默认区分“需求澄清与计划设计”和“按计划执行”两个阶段。除非用户明确要求直接改代码，否则先进入阶段 1。
...
5. 阶段 1 默认不做业务代码实现；只有用户确认进入执行阶段后，才开始阶段 2。

### 阶段 2：按计划拆分执行

目标：严格按阶段 1 的计划落地，由 Codex 负责拆分、调度、复核和集成。

执行规则：

1. Codex 先把计划拆成足够小的执行任务，每个任务应有清晰输入、输出、文件范围和验收命令。
2. 具体实现优先调用 `claude -p` 执行；调用时使用提权执行，避免因沙箱或权限限制导致任务失败。
3. 拆给 Claude 的任务必须足够小，避免长时间卡住；每个任务聚焦一个 repo、一个模块或一个明确步骤。
4. 可以并行的任务并行调用 Claude；存在依赖关系的任务按顺序执行。
5. Codex 不直接信任执行结果，必须复核 diff、运行验证命令，并对照计划逐项确认。
6. 如果 Claude 执行失败或输出不完整，Codex 负责缩小任务、补充上下文后重试，或接手完成小范围修正。
