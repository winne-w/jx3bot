...
以后修改本项目时，默认区分“需求澄清与计划设计”和“按计划执行”两个阶段。该流程适用于实现、重构、排查修复、文档联动等仓库任务。除非用户明确要求直接改代码，或明确表示“开始执行”“进入阶段 2”“按计划实现”“执行修改”等同义指令，否则先进入阶段 1。
...
目标：严格按阶段 1 的计划落地，由主 agent 负责拆分、调度、复核和集成。

执行规则：

1. 主 agent 先把计划拆成足够小的执行任务，每个任务应有清晰输入、输出、文件范围和验收命令。
2. 具体实现分发给子 agent 并行执行，子 agent 负责实际修改文件并完成验证。
   - 派发子 agent 时必须附加类似 `--dangerously-skip-permissions` 的提权参数，确保子 agent 不会被权限确认阻塞，能正常落盘修改文件。
   - 派发子 agent 时必须给出明确执行说明，包含文件范围、期望改动、验收命令或验收标准，明确要求实际修改文件。
   - 不得使用”分析一下””看一下”这类含糊提示交付执行任务，避免子 agent 只做分析不落盘。
3. 拆给子 agent 的任务必须足够小，避免长时间卡住；每个任务聚焦一个 repo、一个模块或一个明确步骤。
4. 可以并行的任务并行派发子 agent；存在依赖关系的任务按顺序执行。
5. 主 agent 不直接信任执行结果，必须复核 diff、运行验证命令，并对照计划逐项确认。
6. 如果子 agent 执行失败或输出不完整，主 agent 负责缩小任务、补充上下文后重试；确需接手时，只接手小范围修正，不扩大改动面。
7. 每完成一个计划步骤，更新执行状态；涉及数据库、API、运行手册或架构边界的改动，按文档更新规则同步相关文档。
8. 完成后给出实现结果、验证结果、未覆盖风险和后续建议。
