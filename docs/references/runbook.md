...
- `属性/装分` 查询的是目标角色最近一场 3v3 对局发生时的装备与属性快照，结果图必须显示“最近 3v3 对局装备快照”、对局时间、总装分/装备分/精炼分/五彩石分、装备与属性；它不代表当前实时面板。
- 成功路径：选择确认有近期 3v3 的角色，执行 `属性 <服务器> <角色>`（或 `装分 <服务器> <角色>`）；确认图片中的服务器、角色名、心法、对局时间和装备/属性与查询目标一致，并确认底部快照声明可见。
- 失败路径：对未知角色、无法补齐身份的角色，或确认没有近期 3v3 的角色执行同一命令；应直接收到原因明确的文本提示（身份不可用、无最近 3v3、详情不可用、目标角色或装备快照缺失），不应发送旧接口的“不可用”提示，也不应回退读取 Mongo 中的历史对局、装备快照或缓存数据。
