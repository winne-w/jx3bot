...
- 已完成 code review；如 review 发现必须处理的问题，已修复、重新验证和再次 review，直到没有必须处理的问题
- 文档同步更新
- 如果改过类型注解、FastAPI 路由签名或启动链路导入模块，额外执行相关文件的 `python -m py_compile`

review 和测试说明：

- review 可由主 agent 自查，也可调用 Codex 子 agent 辅助执行
- 测试、冒烟和验证也可调用 Codex 子 agent 辅助执行
- 使用子 agent 时，主 agent 仍需汇总结果并确认问题已处理
