...
2026-07-29 已收到真实 QQ 运行日志：角色身份、推栏历史和详情请求成功，但详情展示名带 `·服务器` 后缀，造成旧版本误报“未找到目标角色”。该缺陷已以共享规范化函数修复，并对同一场详情 `263952326` 验证能够匹配 `桃桃白糖·唯我独尊`（12 件装备）。修复后的 QQ 成功图片和失败提示仍需重启/热加载 bot 后按下列路径复测。

同日已完成展示口径调整：总装分计算修正为装备分、精炼分、五彩石分之和；不展示分项、五彩石槽位或属性。属性区只展示需求白名单中上游实际返回的字段，气血取 `max_hp`。118 项相关回归测试通过；修复后的 QQ 图片仍需重启/热加载 bot 后按下列路径复测。
