...
7. 暂停 JJC 同步任务，执行小批量 `--apply --limit 20`，抽查结果（已直接全量执行，未单独做小批量抽查）。
8. ~~无异常后全量 apply，保留备份批次号。~~ ✅
9. 恢复 JJC 同步任务，观察后续新增 `role_identities.name` 是否仍出现 `·服务器`（如执行前未暂停同步则可跳过恢复，直接进入观察期）。

## 当前状态

- 阶段：线上修复已执行完毕，进入后续观察期。
- 业务代码、单元测试、数据库文档已提交（`828be52`）。
- 历史修复脚本已全量 `--apply` 执行，临时脚本执行后已删除。
- 待确认：若执行修复时暂停了 JJC 同步任务，需恢复。观察后续写入不再出现 `·服务器` 角色名。
