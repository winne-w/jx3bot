...
这里存放阶段性计划和技术债推进记录，按"进行中 / 已完成"组织。新需求默认优先使用 `docs/requirements/` 目录；`docs/exec-plans/` 主要承载历史计划、小范围修复、技术债和续做项。计划内实现、验证和 review 完成后，必须在提交代码前把计划从 `active/` 移动到 `completed/` 并同步更新本索引。
