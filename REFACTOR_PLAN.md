...
1) 启动与初始化
- 启动后观察日志：`cache_init` 不应出现 `'CombinedDriver' object has no attribute 'state'`
- 启动后观察日志：`wanbaolou` 的 `check_timeouts` 不应频繁出现 APScheduler misfire 警告

2) 帮助与基础查询
- 群内发送：`帮助`（能正常渲染/返回）
- 群内发送：`更新` / `活动` / `技改`（能拉取公告并返回文本）

3) 交易行
- 群内发送：`交易行 <物品名>`（能返回价格图/文本，`fetch_json` 链路正常）

4) 名片
- 群内发送：`名片 <角色>` / `名片 <服务器> <角色>`（首次应下载并返回图片）
- 再次发送同样命令（命中缓存时也应直接返回 `查询结果` + 图片，不应再出现 NapCat `copyfile ... ENOENT`）

5) 竞技排名
- 群内发送：`竞技排名`（合并图 + 摘要）
- 群内发送：`竞技排名 拆分`（多张拆分图 + 摘要）
- 群内发送：`竞技排名 debug`（应分段输出调试列表）

6) status_monitor（如启用）
- 开服/新闻/技改/福利/日常/竞技排名推送开关命令可用：`/开服推送 开启` 等
- 定时推送能正常发图/文本（尤其每日竞技排名推送）
